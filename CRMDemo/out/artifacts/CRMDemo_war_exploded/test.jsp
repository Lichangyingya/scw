...
<head>
    ...
    <script type="text/javascript">
        $(function () {
            var pageNumber = 1;
            var url = "pageStaffs"
            var args = {"pageNumber":pageNumber,"time":new Date()}
            getJson(url,args);
        ...
        function getJson(url,args) {
            ...
            $.getJSON(url,args,function (data) {
                if (data != null) {
                  $.each(data.pageInfo.list,function (i,staff) {
                      $(".dataTable").append("<tr>" +
                                                "<td>"+staff.staffName+"</td>"+
                                                "<td>"+staff.gender+"</td>"+
                                                "<td>"+new Date(staff.onDutyDate ).getDate()+"</td>"+
                                                "<td>"+staff.depName+"</td>"+
...
<div id="main">
    ...
    <span id="pageSpanId"><br>
        <a href="${pageContext.request.contextPath}/showStaff?pageNumber=1">&nbsp;&nbsp;首页</a>
         <c:if test="${pageInfo.pageNumber>1}">
             <a href="${pageContext.request.contextPath}/showStaff?pageNumber=${pageInfo.pageNumber-1}">&nbsp;上一页</a>
         </c:if>
         <c:if test="${pageInfo.pageNumber<pageInfo.totalPage}">
             <a href="${pageContext.request.contextPath}/showStaff?pageNumber=${pageInfo.pageNumber+1}">&nbsp;下一页</a>
         </c:if>
        <a href="${pageContext.request.contextPath}/showStaff?pageNumber=${pageInfo.totalPage}">&nbsp;尾页</a>
