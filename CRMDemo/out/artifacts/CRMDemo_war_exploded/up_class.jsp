...
<div id="main">
    <div id="topMenu">
        ...
        <a  id="upCLass" href="javascript:inputCheck('已升学')">
            <img src="images/button/shengban.gif">
        </a>
        <a id="changeClass" href="javascript:inputCheck('已留级')">
            <img src="images/button/liuji.gif">
        </a>
        <button class="btn" onClick="query()"/>
    </div>
    <div id="selectFrom">
        <form action="advancedStudentQuery" id="formId" method="get">
        ...
        <table class="dataTable" width="100%" border="1" cellspacing="0" cellpadding="0">
            ...
            <c:forEach items="${pageInfo.list}" var="student">
                <tr>
                    ...
                    <td>${student.crmClass.className}</td>
    ...
    <div id="pageDiv">
    <span id="pageSpanId">
         <a href="${pageContext.request.contextPath}/upgrade?pageNumber=1">&nbsp;&nbsp;首页</a>
         <c:if test="${pageInfo.pageNumber>1}">
             <a href="${pageContext.request.contextPath}/upgrade?pageNumber=${pageInfo.pageNumber-1}">&nbsp;上一页</a>
         </c:if>
         <c:if test="${pageInfo.pageNumber<pageInfo.totalPage}">
             <a href="${pageContext.request.contextPath}/upgrade?pageNumber=${pageInfo.pageNumber+1}">&nbsp;下一页</a>
         </c:if>
        <a href="${pageContext.request.contextPath}/upgrade?pageNumber=${pageInfo.totalPage}">&nbsp;尾页</a>
        ...
        function inputCheck(status){
            ...
            if(flag==false){// !flag
                ...
                for (var i = 0; i <chk_value.length ; i++) {
                    checkboxs += chk_value[i]+","
                }
                var args = {"checkboxs":checkboxs,"status":status,"time":new Date()}
                $.get("changeStatus",args,function (data) {
                    console.log(data)
                    if(data ==chk_value.length){
